...
#include "allgamescoinaddressvalidator.h"
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    //
    //    Cannot handle this later, because allgamescoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("allgamescoin://", Qt::CaseInsensitive))
    {
...
void openConfigfile()
{
    ...
    /* Open allgamescoin.conf with the associated application */
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "allgamescoincore.desktop";
    return GetAutostartDir() / strprintf("allgamescoincore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a allgamescoincore.desktop file to the autostart directory:
...
bool GetStartOnSystemStartup()
{
    CFURLRef allgamescoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
