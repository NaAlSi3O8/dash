...
const char * const ALLGAMESCOIN_CONF_FILENAME = "allgamescoin.conf";
const char * const ALLGAMESCOIN_PID_FILENAME = "allgamescoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "allgamescoin" is a composite category enabling all Allgamescoin-related debug output
            if(ptrCategory->count(string("allgamescoin"))) {
...
#else
    const char* pszModule = "allgamescoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AllgamescoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AllgamescoinCore
    // Mac: ~/Library/Application Support/AllgamescoinCore
    // Unix: ~/.allgamescoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AllgamescoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AllgamescoinCore";
#else
    // Unix
    return pathRet / ".allgamescoincore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", ALLGAMESCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty allgamescoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override allgamescoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", ALLGAMESCOIN_PID_FILENAME));
