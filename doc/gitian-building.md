...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-allgamescoin-host/README.md).
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/allgamescoin/.git/
...
For example:
```bash
URL=https://github.com/crowning-/allgamescoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit allgamescoin=${COMMIT} --url allgamescoin=${URL} ../allgamescoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit allgamescoin=${COMMIT} --url allgamescoin=${URL} ../allgamescoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit allgamescoin=${COMMIT} --url allgamescoin=${URL} ../allgamescoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the allgamescoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../allgamescoin/contrib/gitian-descriptors/*|sort|uniq )
...
XAGCPATH=/some/root/path/allgamescoin.git
SIGPATH=/some/root/path/allgamescoin-detached-sigs.git

./bin/gbuild --url allgamescoin=${XAGCPATH},signature=${SIGPATH} ../allgamescoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/allgamescoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/allgamescoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/allgamescoin-osx-build.assert
```
