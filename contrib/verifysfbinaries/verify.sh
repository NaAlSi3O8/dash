...
###   This script attempts to download the signature file SHA256SUMS.asc from allgamescoin.org
...
WORKINGDIR="/tmp/allgamescoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://allgamescoin.org/bin/"
VERSIONPREFIX="allgamescoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'allgamescoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the AllGamesCoin Core binary release signing key installed?"
   fi
