...
{
    return QCoreApplication::translate("allgamescoin-core", psz).toStdString();
}
...
/** Class encapsulating AllGamesCoin startup and shutdown.
...
void AllGamesCoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // allgamescoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", AllGamesCoinGUI::tr("A fatal error occurred. AllGamesCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse allgamescoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        QMessageBox::critical(0, QObject::tr("AllGamesCoin"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("AllGamesCoin"), QObject::tr("Error: %1").arg(e.what()));
        return EXIT_FAILURE;
    }
#ifdef ENABLE_WALLET
    ...
    // allgamescoin: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("AllGamesCoin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
